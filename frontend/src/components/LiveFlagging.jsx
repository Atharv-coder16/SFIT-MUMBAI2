...
import { getAtRiskCustomers, getCustomerDetail, getCustomerHistory, explainCustomer, getCustomerTimeline, getAbilityWillingness, getCustomerContext, getLatestStream } from '../api/client';
...
// ═══════════════════════════════════════════
// CUSTOMER RADAR VIEW — Per-Customer Risk Radar
// ═══════════════════════════════════════════
function CustomerRadarView({ normalized, detail, contextData, explanation, d }) {
  const radarRef = useRef(null);
  const pulseRef = useRef(null);
  const [hoveredAxis, setHoveredAxis] = useState(null);

  const riskScore = normalized.ensemble_prob || normalized.risk_score || 0;
  const signals = d?.recent_signals || explanation?.signals || [];
  const shapDrivers = explanation?.all_drivers || explanation?.top_drivers || d?.all_shap || d?.shap_top3 || [];

  // Build radar axes from SHAP attributions (real model data)
  const radarAxes = useMemo(() => {
    if (shapDrivers.length > 0) {
      return shapDrivers.slice(0, 6).map(s => ({
        axis: (s.feature || '').replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
        value: Math.min(100, Math.abs(s.contribution ?? s.abs_contribution ?? s.value ?? 0) * 200),
        raw: s.contribution ?? s.abs_contribution ?? s.value ?? 0,
        color: (s.contribution ?? s.value ?? 0) >= 0 ? '#ff4757' : '#06ffa5',
      }));
    }
    // Fallback: build from behavioral features
    const cu = normalized.credit_utilization || 0;
    return [
      { axis: 'Credit Util.', value: Math.min(100, cu * 100), raw: cu, color: cu > 0.7 ? '#ff4757' : '#06ffa5' },
      { axis: 'Salary Risk', value: signals.some(s => s.toLowerCase().includes('salary')) ? 85 : 15, raw: 0, color: '#ff6b35' },
      { axis: 'Autodebit', value: signals.some(s => s.toLowerCase().includes('autodebit') || s.toLowerCase().includes('failed')) ? 80 : 10, raw: 0, color: '#f59e0b' },
      { axis: 'UPI Lending', value: signals.some(s => s.toLowerCase().includes('upi') || s.toLowerCase().includes('lender')) ? 75 : 10, raw: 0, color: '#a78bfa' },
      { axis: 'EMI Burden', value: normalized.emi_amount && normalized.monthly_salary ? Math.min(100, (normalized.emi_amount / normalized.monthly_salary) * 100) : 30, raw: 0, color: '#00d4ff' },
      { axis: 'ATM Activity', value: signals.some(s => s.toLowerCase().includes('atm')) ? 70 : 10, raw: 0, color: '#06ffa5' },
    ];
  }, [shapDrivers, signals, normalized]);

  // ── Radar Canvas ──
  useEffect(() => {
    if (!radarRef.current || !radarAxes.length) return;
    ...
    const draw = () => {
      ...
      // Axis lines + labels
      radarAxes.forEach((d, i) => {
        const a = (i / n) * Math.PI * 2 - Math.PI / 2;
        ctx.beginPath(); ctx.moveTo(cx, cy);
        ctx.lineTo(cx + Math.cos(a) * maxR, cy + Math.sin(a) * maxR);
        ctx.strokeStyle = hoveredAxis === i ? 'rgba(0,212,255,0.35)' : 'rgba(255,255,255,0.05)';
        ctx.stroke();
        ...
        if (hoveredAxis === i) {
          ...
          ctx.fillText(`${Math.round(d.value * ease)}%`, lx, ly + 13);
        }
      });
      ...
      ctx.shadowColor = '#ff475780'; ctx.shadowBlur = 12;
      ...
      radarAxes.forEach((d, i) => {
        ...
        const r = (d.value / 100) * maxR * ease;
        ctx.beginPath();
        ctx.arc(cx + Math.cos(a) * r, cy + Math.sin(a) * r, hoveredAxis === i ? 5 : 3.5, 0, Math.PI * 2);
        ctx.fillStyle = d.color;
      ...
      // Center label
      ctx.fillStyle = riskScore >= 0.7 ? '#ff4757' : riskScore >= 0.4 ? '#ff6b35' : '#06ffa5';
      ctx.font = 'bold 26px "Syne", sans-serif'; ctx.textAlign = 'center';
      ctx.fillText(`${(riskScore * 100).toFixed(0)}`, cx, cy - 2);
      ctx.fillStyle = '#5a5a7a'; ctx.font = '8px "DM Sans", sans-serif';
  ...
  }, [radarAxes, hoveredAxis, riskScore]);
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const baseY = H / 2;
      const intensity = riskScore;
      const color = riskScore >= 0.7 ? '#ff4757' : riskScore >= 0.4 ? '#ff6b35' : '#06ffa5';
      ...
      for (let x = 0; x < W; x++) {
        ...
        let y = baseY;
        if (beat > 0.8 && beat < 1.2) y -= Math.sin((beat - 0.8) * Math.PI / 0.4) * 8 * intensity;
        else if (beat > 1.5 && beat < 2.0) {
          const q = (beat - 1.5) / 0.5;
          if (q < 0.3) y += q * 90 * intensity;
          else if (q < 0.5) y -= (q - 0.3) * 220 * intensity;
          else if (q < 0.7) y += (q - 0.5) * 150 * intensity;
          else y -= (q - 0.7) * 30 * intensity;
        } else if (beat > 2.5 && beat < 3.0) y -= Math.sin((beat - 2.5) * Math.PI / 0.5) * 10 * intensity;
        y += (Math.random() - 0.5) * 1;
  ...
  const handleRadarMouse = (e) => {
    const rect = radarRef.current.getBoundingClientRect();
    const mx = e.clientX - rect.left, my = e.clientY - rect.top;
    const angle = Math.atan2(my - 150, mx - 150);
    let norm = (angle + Math.PI / 2) / (Math.PI * 2);
    if (norm < 0) norm += 1;
    const idx = Math.round(norm * radarAxes.length) % radarAxes.length;
    const dist = Math.sqrt((mx - 150) ** 2 + (my - 150) ** 2);
    setHoveredAxis(dist < 140 ? idx : null);
  };

  const pulseColor = riskScore >= 0.7 ? '#ff4757' : riskScore >= 0.4 ? '#ff6b35' : '#06ffa5';

  return (
    <div style={{ padding: '0 16px' }}>
      {/* Row 1 — Radar + Signal Heatmap */}
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: 16, marginBottom: 16 }}>
        ...
        <div style={{
          ...
          <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 12, color: '#e0e0f0', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '1px' }}>
            🔴 Individual Risk Radar
          </div>
          <canvas ref={radarRef} style={{ width: 300, height: 300, cursor: 'crosshair' }}
            onMouseMove={handleRadarMouse} onMouseLeave={() => setHoveredAxis(null)} />
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 12, color: '#e0e0f0', marginBottom: 12, textTransform: 'uppercase', letterSpacing: '1px' }}>
            ⚡ Signal Strength Matrix
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {radarAxes.map((ax, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, animation: `fadeSlideLeft 300ms ease ${i * 60}ms both` }}>
                <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 11, color: '#9999bb', width: 100, flexShrink: 0 }}>
                  {ax.axis}
                </span>
                <div style={{ flex: 1, height: 16, borderRadius: 4, background: 'rgba(255,255,255,0.04)', overflow: 'hidden', position: 'relative' }}>
                  <div style={{
                    ...
                    width: `${ax.value}%`,
                    background: `linear-gradient(90deg, ${ax.color}40, ${ax.color})`,
                    boxShadow: `0 0 8px ${ax.color}30`,
                    transition: 'width 0.8s ease-out',
                  }} />
                  <span style={{
                    position: 'absolute', right: 6, top: '50%', transform: 'translateY(-50%)',
                    fontFamily: "'DM Mono', monospace", fontSize: 9, color: '#e0e0f0', fontWeight: 600,
                  }}>
                    {Math.round(ax.value)}%
                  </span>
                </div>
          ...
          {contextData && (
            <div style={{ marginTop: 16 }}>
              <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 11, color: '#5a5a7a', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '1px' }}>
                Context Engine
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6 }}>
                {[
                  { label: 'Type', value: contextData.user_type, color: '#00d4ff' },
                  { label: 'Season', value: contextData.season, color: '#f59e0b' },
                  { label: 'Stability', value: `${(contextData.income_stability * 100).toFixed(0)}%`, color: contextData.income_stability > 0.5 ? '#06ffa5' : '#ff4757' },
                  { label: 'Engagement', value: `${contextData.engagement_score}/100`, color: contextData.engagement_score > 50 ? '#06ffa5' : '#ff6b35' },
                  { label: 'Anomaly', value: contextData.anomaly_type, color: contextData.anomaly_type === 'NONE' ? '#06ffa5' : '#ff4757' },
                  { label: 'Confidence', value: `${((contextData.context_confidence || 0) * 100).toFixed(0)}%`, color: '#a78bfa' },
                ].map((item, i) => (
                  <div key={i} style={{
                    background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)',
                    borderRadius: 6, padding: '4px 6px', textAlign: 'center',
                  }}>
                    <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, color: item.color, fontWeight: 600 }}>{item.value}</div>
                    <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 8, color: '#5a5a7a', textTransform: 'uppercase' }}>{item.label}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* Row 2 — Heartbeat Pulse */}
      <div style={{
        background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)',
        borderRadius: 12, padding: 16,
      }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <span style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 12, color: '#e0e0f0', textTransform: 'uppercase', letterSpacing: '1px' }}>
            💓 Stress Pulse — {normalized.name}
          </span>
          <span style={{
            fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 20, color: pulseColor,
            animation: 'breathe 1.5s ease-in-out infinite',
          }}>
            {(riskScore * 100).toFixed(1)}%
          </span>
        </div>
        <canvas ref={pulseRef} style={{ width: '100%', height: 100, borderRadius: 8 }} />
        <div style={{ display: 'flex', gap: 12, marginTop: 8, justifyContent: 'center' }}>
          {[
            { label: 'Product', value: normalized.product_type || '—', color: '#00d4ff' },
            { label: 'City', value: normalized.city || '—', color: '#a78bfa' },
            { label: 'EMI/Salary', value: normalized.monthly_salary ? `${((normalized.emi_amount / normalized.monthly_salary) * 100).toFixed(0)}%` : '—', color: (normalized.emi_amount / (normalized.monthly_salary || 1)) > 0.5 ? '#ff4757' : '#06ffa5' },
            { label: 'Credit Score', value: normalized.credit_score || '—', color: (normalized.credit_score || 700) < 650 ? '#ff4757' : '#06ffa5' },
          ].map((item, i) => (
            <div key={i} style={{
              background: `${item.color}08`, border: `1px solid ${item.color}20`,
              padding: '4px 12px', borderRadius: 6, textAlign: 'center',
            }}>
              <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, fontWeight: 600, color: item.color }}>{item.value}</div>
              <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 8, color: '#5a5a7a', textTransform: 'uppercase' }}>{item.label}</div>
            </div>
          ))}
        </div>
...
function CustomerModal({ customerId, onClose }) {
  ...
  const [modalTab, setModalTab] = useState('profile'); // 'profile' | 'radar'
  ...
  useEffect(() => {
    Promise.all([
      ...
      getCustomerContext(customerId).catch(() => null)
    ]).then(([d, h, e, t, aw, ctx]) => {
      setDetail(d);
      setHistory(h || []);
      setExplanation(e);
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-card" onClick={e => e.stopPropagation()} style={{ maxWidth: 960 }}>
        ...
        <div style={{
          ...
          {['profile', 'radar'].map(tab => (
            <button key={tab} onClick={() => setModalTab(tab)} style={{
              flex: 1, padding: '10px 0', cursor: 'pointer',
              fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 12,
              textTransform: 'uppercase', letterSpacing: '1.5px',
              background: modalTab === tab
                ? (tab === 'radar' ? 'rgba(255,71,87,0.06)' : 'rgba(0,212,255,0.06)')
                : 'transparent',
              color: modalTab === tab
                ? (tab === 'radar' ? '#ff4757' : '#00d4ff')
                : '#5a5a7a',
              border: 'none',
              borderBottom: modalTab === tab
                ? `2px solid ${tab === 'radar' ? '#ff4757' : '#00d4ff'}`
                : '2px solid transparent',
              transition: 'all 200ms ease',
            }}>
              {tab === 'profile' ? '📋 Profile & Analysis' : '🔴 Risk Radar'}
            </button>
          ))}
        </div>
        ...
        {/* ─── RADAR TAB ─── */}
        {modalTab === 'radar' && (
          <div style={{ padding: '16px 0', animation: 'fadeSlideUp 400ms ease' }}>
            <CustomerRadarView
        ...
        <div className="modal-body">
          ...
          <div>
            ...
            {contextData && (
              <div style={{ marginTop: 16 }}>
                <div className="section-label">Context Intelligence Engine</div>
                <div style={{
                  display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8,
                  marginBottom: 12,
                }}>
                  {[
                    { label: 'User Type', value: contextData.user_type, icon: '👤', color: '#00d4ff' },
                    { label: 'Season', value: contextData.season, icon: '📅', color: '#f59e0b' },
                    { label: 'Engagement', value: `${contextData.engagement_score}/100`, icon: '📱', color: contextData.engagement_score > 50 ? '#06ffa5' : '#ff6b35' },
                    { label: 'Income Stability', value: `${(contextData.income_stability * 100).toFixed(0)}%`, icon: '💰', color: contextData.income_stability > 0.5 ? '#06ffa5' : '#ff4757' },
                    { label: 'Anomaly Type', value: contextData.anomaly_type, icon: '⚡', color: contextData.anomaly_type === 'NONE' ? '#06ffa5' : contextData.anomaly_type === 'MULTI_SIGNAL' ? '#ff4757' : '#ff6b35' },
                    { label: 'Context Conf.', value: `${((contextData.context_confidence || 0) * 100).toFixed(0)}%`, icon: '🎯', color: (contextData.context_confidence || 0) > 0.7 ? '#06ffa5' : '#ff6b35' },
                  ].map((item, i) => (
                    <div key={i} style={{
                      background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)',
                      ...
                      <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 9, color: '#5a5a7a', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                        {item.label}
                      </div>
                    </div>
                  ))}
                ...
                {contextData.seasonal_dampening !== 0 && (
                  <div style={{
                    padding: '6px 10px', borderRadius: 6, marginBottom: 8,
                    background: 'rgba(0,212,255,0.06)', border: '1px solid rgba(0,212,255,0.15)',
                    fontFamily: "'DM Mono', monospace", fontSize: 11, color: '#7dd3fc',
                  }}>
                    📊 Seasonal Adjustment: {contextData.seasonal_dampening > 0 ? '+' : ''}{(contextData.seasonal_dampening * 100).toFixed(1)}% (context: {contextData.season})
                  </div>
                )}

                {/* Edge Case Alerts */}
                {contextData.edge_cases && contextData.edge_cases.filter(ec => ec.type !== 'HIDDEN_BORROWING').length > 0 && (
                  <div style={{ display: 'grid', gap: 6 }}>
                    {contextData.edge_cases.filter(ec => ec.type !== 'HIDDEN_BORROWING').map((ec, i) => {
                      const sevColor = ec.severity === 'HIGH' ? '#ff4757' : ec.severity === 'MEDIUM' ? '#ff6b35' : '#f59e0b';
                      return (
                        <div key={i} style={{
                          background: `${sevColor}08`, border: `1px solid ${sevColor}25`,
                          borderLeft: `3px solid ${sevColor}`, borderRadius: 8, padding: '10px 12px',
                          animation: ec.severity === 'HIGH' ? 'redPulse 2s infinite' : 'none',
                        }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                            <span style={{
                              fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 12, color: sevColor,
                            }}>
                              ⚠ {ec.name}
                            </span>
                            <span style={{
                              fontFamily: "'DM Mono', monospace", fontSize: 9, padding: '2px 6px',
                              borderRadius: 4, background: `${sevColor}15`, color: sevColor,
                            }}>
                              {ec.severity} • {(ec.confidence * 100).toFixed(0)}% conf
                            </span>
                          </div>
                          <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 11, color: '#b0b0cc', lineHeight: 1.5 }}>
                            {ec.description}
                          </div>
                          <div style={{
                            marginTop: 4, fontFamily: "'DM Mono', monospace", fontSize: 10,
                            color: '#06ffa5', background: 'rgba(6,255,165,0.06)',
                            padding: '3px 6px', borderRadius: 4, display: 'inline-block',
                          }}>
                            → {ec.action}
                          </div>
                        </div>
